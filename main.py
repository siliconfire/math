  ...
  if choice == "1":
    ...
    choiceThree()
      ...
      if devOptions:
        ...
        printflush = "Hidden settings is Disabled! Enable it first to enable debug."
          
  elif choice == "5":
      if devOptions:
          if not debug:
            devOptions = False
            printflush = "Hidden settings Disabled."
          else:
            printflush = "Turn OFF Debug with [4] first."
      else:
        printflush = "Hidden settings is already Disabled!"
      ...
      if not devOptions:
        ...
        os.system("clear")
        print("Hidden settings Enabled!\n\nYou have just enabled Hidden settings, which will probably do nothing except for allowing you to enable debug mode. This will be changed in a further update, i have other things to do right now. \n\nTo disable Hidden setings, select [5] at the main menu.\n\n")
        input("[CONTINUE]")

      else:
        printflush = "Hidden settings is already Enabled!"
  else:
    printflush = "Invalid input (ValueError)"
